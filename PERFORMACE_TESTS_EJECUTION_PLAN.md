# DevMetrics Live — v2 Implementation Plan
...
3. **Gem Packaging** — Structure the app as a mountable Rails Engine gem (`devmetrics_live`) with an install generator.
...
- The job:
  1. Scans `spec/requests/**/*.rb` for files that `require 'devmetrics_live'` or include `DevmetricsLive::PerformanceHelpers`.
  ...
  4. Detects N+1s via Bullet (already configured) → records [SlowQuery](file:///home/starklord/Projects/rails/devmetrics_live/app/javascript/controllers/metrics_controller.js#84-109) rows with fix suggestions.
  5. Broadcasts progress events (start, each test line, summary, end) to [MetricsChannel](file:///home/starklord/Projects/rails/devmetrics_live/app/channels/metrics_channel.rb#1-10).
- Remove the 5-second polling (`setInterval` in [metrics_controller.js](file:///home/starklord/Projects/rails/devmetrics_live/app/javascript/controllers/metrics_controller.js)).

### Files to change/add

#### [MODIFY] [metrics_controller.rb](file:///home/starklord/Projects/rails/devmetrics_live/app/controllers/metrics_controller.rb)
...
- Broadcasts each line of output to [MetricsChannel](file:///home/starklord/Projects/rails/devmetrics_live/app/channels/metrics_channel.rb#1-10) (`type: "test_output"`).
...
#### [MODIFY] [metrics_channel.rb](file:///home/starklord/Projects/rails/devmetrics_live/app/channels/metrics_channel.rb)
- Wire up `stream_from "metrics_channel"`.

#### [MODIFY] [routes.rb](file:///home/starklord/Projects/rails/devmetrics_live/config/routes.rb)
- Add `post '/devmetrics/run_tests', to: 'metrics#run_tests'`.

#### [MODIFY] [metrics/index.html.erb](file:///home/starklord/Projects/rails/devmetrics_live/app/views/metrics/index.html.erb)
- Add **"Run Performance Tests"** CTA button.
- Add a live terminal-style output panel (hidden by default, shown during/after run).

#### [MODIFY] [metrics_controller.js](file:///home/starklord/Projects/rails/devmetrics_live/app/javascript/controllers/metrics_controller.js)
...
```
lib/
  devmetrics_live.rb              # Main entry point + autoloads
  devmetrics_live/
    engine.rb                     # Rails::Engine, isolate_namespace
    version.rb                    # VERSION = "0.1.0"
    performance_helpers.rb        # Optional test helper for host apps

devmetrics_live.gemspec           # Gem metadata + dependencies

lib/generators/
  devmetrics_live/
    install/
      install_generator.rb        # `rails g devmetrics_live:install`
      templates/
        devmetrics_live.rb        # initializer template
        migration_template.rb     # Creates query_logs + slow_queries tables
```

### Engine Design
- `isolate_namespace DevmetricsLive` — no route/model conflicts.
- Routes mounted with `mount DevmetricsLive::Engine, at: '/devmetrics'` in the host app.
...
```
Creates:
1. `config/initializers/devmetrics_live.rb` (configurable threshold, cable adapter).
...
Gem::Specification.new do |s|
  s.name        = "devmetrics_live"
  s.version     = DevmetricsLive::VERSION
...
### Automated
- `bin/rails runner 'puts DevmetricsLive::VERSION'` — gem loaded.
- `bin/rails g devmetrics_live:install` — generator produces files.
